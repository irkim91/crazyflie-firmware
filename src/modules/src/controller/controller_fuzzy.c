...
{
  fuzzyControllerInit(ATTITUDE_UPDATE_DT);
}
                                         ...
                                         const stabilizerStep_t stabilizerStep)
{
  ...
  if (RATE_DO_EXECUTE(ATTITUDE_RATE, stabilizerStep)) {
    ...
    if (setpoint->mode.yaw == modeVelocity) {
      ...
      float yawMaxDelta = fuzzyattitudeControllerGetYawMaxDelta();
      if (yawMaxDelta != 0.0f)
      {
  ...
  if (RATE_DO_EXECUTE(POSITION_RATE, stabilizerStep)) {
    fuzzypositionController(&actuatorThrust, &attitudeDesired, setpoint, state);
  }
  ...
  if (RATE_DO_EXECUTE(ATTITUDE_RATE, stabilizerStep)) {
    ...
    attitudeControllerCorrectAttitudefuzzy(state->attitude.roll, state->attitude.pitch, state->attitude.yaw,
    ...
    if (setpoint->mode.roll == modeVelocity) {
      ...
      attitudeControllerResetRollAttitudefuzzy(state->attitude.roll);
    }
    if (setpoint->mode.pitch == modeVelocity) {
      rateDesired.pitch = setpoint->attitudeRate.pitch;
      attitudeControllerResetPitchAttitudefuzzy(state->attitude.pitch);
    }

    // TODO: Investigate possibility to subtract gyro drift.
    attitudeControllerCorrectRatefuzzy(sensors->gyro.x, -sensors->gyro.y, sensors->gyro.z,
                             rateDesired.roll, rateDesired.pitch, rateDesired.yaw);

    fuzzyattitudeControllerGetActuatorOutput(&control->roll,
                                        &control->pitch,
                                        &control->yaw);
  ...
  if (control->thrust == 0)
  {
    ...
    attitudeControllerResetAllfuzzy(state->attitude.roll, state->attitude.pitch, state->attitude.yaw);
    positionControllerResetAllfuzzy(state->position.x, state->position.y, state->position.z);
